...
public class Climber extends SubsystemBase {
  ...
  public final CANSparkMax hookMotor = new CANSparkMax(Constants.HOOK_MOTOR, MotorType.kBrushless);
  public final CANSparkMax barMotor = new CANSparkMax(Constants.BAR_MOTOR, MotorType.kBrushless);
  private SparkMaxPIDController barPID, hookPID;
  ...
  public void setArmVolt(double percent) {
    barMotor.set(percent);
  }

  public void manualBarMotorControl() {
    double rawAxisValue = m_stick.getRawAxis(Constants.RIGHT_STICK_Y_AXIS);
    double inc = (Math.abs(rawAxisValue) < Constants.JOYSTICK_DRIFT_TOLERANCE ? 0 : rawAxisValue)
        * Constants.ARM_SPEED_MULTIPLIER;
    // armPosition = Math.min(Math.max(armPosition + inc, Constants.MIN_ARM_POS),
    // Constants.MAX_ARM_POS);
    armPosition = Math.min(armPosition + inc, Constants.MAX_ARM_POS);
  ...
  public void manualHookMotorControl() {
    ...
    setHook(inc * Constants.HOOK_SPEED_MULTIPLIER);
  }
  ...
  public void setHookPosition(double position) {
    ...
    // System.out.println(armSetp);
    hookPID.setReference(position, ControlType.kSmartMotion);
  }
  ...
  private void initValues() {
    ...
    hookPID.setI(kI);
    hookPID.setD(kD);
    hookPID.setIZone(kIz);
    hookPID.setFF(kFF);
    hookPID.setOutputRange(kMinOutput, kMaxOutput);
